...
class SGLR(GeneralGraphRecommender):
    r"""SGL是一个基于GCN的推荐模型。
    ...
    def __init__(self, config, dataset):
        super(SGLR, self).__init__(config, dataset)
    ...
    def random_graph_augment_ea(self):
        ...
        def sample(size=None, replace=True):
            items = []
            users = []
            # return np.random.choice(np.arange(high), size=size, replace=replace)
            i = 0
        ...
        u, v = sample(size=int(len(self._user) * self.drop_ratio), replace=False)
        v = v + self.n_users
