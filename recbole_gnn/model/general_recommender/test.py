...
class TEST(GeneralGraphRecommender):
    r"""SGL是一个基于GCN的推荐模型。
    ...
    def __init__(self, config, dataset):
        ...
        self._user_set = set(dataset.inter_feat[dataset.uid_field].tolist())  # user列
        self._item = dataset.inter_feat[dataset.iid_field]  # item列
        self._item_set = set(dataset.inter_feat[dataset.iid_field].tolist())  # item列

        self.uid2iids = {}
        e_i1 = self._user.tolist()
        e_i2 = self._item.tolist()
        for j in range(len(e_i1)):
            self.uid2iids.setdefault(e_i1[j], []).append(e_i2[j])
    ...
    def graph_construction(self):
        r"""Devise three operators to generate the views — node dropout, edge dropout, and random walk of a node.
            实验1： 随机增加边
        """
        self.sub_graph1 = [self.random_graph_augment_ea()] * self.n_layers
        self.sub_graph2 = [self.random_graph_augment_ea()] * self.n_layers
    ...
    def random_graph_augment_ea(self):
        def rand_sample(size=None, replace=True):
            items = []
            users = []
            # return np.random.choice(np.arange(high), size=size, replace=replace)
            i = 0
            while i < size:
                # item = np.random.choice(self._item_set, 1)
                # user = np.random.choice(self._user_set, 1)
                user = np.random.randint(1, self.n_users)
                item = np.random.randint(1, self.n_items)
                if item in self.uid2iids[user]:
                    continue
                else:
                    i += 1
                    items.append(item)
        ...
        col = self._item + self.n_users  # 在_user和_item上进行采样其实就是随机丢失了边
        edge_index1 = torch.stack([row, col])
        edge_index2 = torch.stack([col, row])
        edge_index = torch.cat([edge_index1, edge_index2], dim=1)  # 这里就是正常的生成 `边索引`, [2, 2 * num_interact]

        # new_dict = {}
        # e_i1 = edge_index[0][:].tolist()
        # e_i2 = edge_index[1][:].tolist()
        # for j in range(len(e_i1)):
        #     new_dict.setdefault(e_i1[j], []).append(e_i2[j])

        u, v = rand_sample(size=int(len(self._user) * self.drop_ratio), replace=False)
        v = v + self.n_users
        edge_index3 = torch.stack([u, v])
        edge_index4 = torch.stack([v, u])
        edge_index_add = torch.cat([edge_index3, edge_index4], dim=1)
        edge_index_end = torch.cat([edge_index, edge_index_add], dim=1)

        deg = degree(edge_index_end[0], self.n_users + self.n_items)  # 计算度, [num+user + num_items, ]
        ...
        edge_weight = norm_deg[edge_index_end[0]] * norm_deg[edge_index_end[1]]

        return edge_index_end.to(self.device), edge_weight.to(self.device)
    ...
    def random_graph_augment(self):
        ...
        if self.aug_type == "ND":  # 节点丢弃
            ...
            keep = rand_sample(len(self._user), size=int(len(self._user) * (1 - self.drop_ratio)),
                               replace=False)  # 按照给定的比例采样保留的inedx
            row = self._user[keep]
            # 把 `item` 的` index` 接到 `user` 后是为了计算度，因为``user`和 `item` 编号默认都是从1开始的，这里的的`user`和`item`与模型运算无关
